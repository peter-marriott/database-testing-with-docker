...
def index():
    ...
    try:
        ...
        cur = conn.cursor()
        try:
            columns = ( 'customerid', 'surname', 'initials', 'firstname', 'title', 'postcode1', 'postcode2', 'sex', 'email', 'passwd' )
            cur.execute("select customerid, surname, initials, firstname, title, postcode1, postcode2, sex, email, passwd from reporting.customer order by surname, firstname")
            customer_list = []
